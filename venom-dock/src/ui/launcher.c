...
void on_launcher_clicked(GtkWidget *widget, gpointer data) {
    ...
    /* Full Screen Size */
    gint screen_width = gdk_screen_get_width(l_screen);
    gint screen_height = gdk_screen_get_height(l_screen);
    ...
    /* Pager Injection */
    {
        /* Ensure Pager Service is Init? It's done in main maybe, but safe to do here */
        GdkDisplay *gd = gdk_display_get_default();
        Display *dpy = GDK_DISPLAY_XDISPLAY(gd);
        Window root = GDK_WINDOW_XID(gdk_screen_get_root_window(l_screen));
...
void create_launcher_button(GtkWidget *box) {
    /* ... Load CSS (omitted for brevity, assume present) ... */
